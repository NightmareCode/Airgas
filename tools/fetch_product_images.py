...
def main():
    ...
    ert_products = [p for p in data['products'] if p['category'] == 'PPE for ERT']
    output_dir = 'assets/PPEForERT'
    temp_dir = 'assets/temp_img'
    
    os.makedirs(output_dir, exist_ok=True)
    os.makedirs(temp_dir, exist_ok=True)
    
    print(f"Starting auto-fetch for {len(ert_products)} PPE for ERT products via Bing.")
    
    for i, product in enumerate(ert_products):
        filename = sanitize_filename(product['name'])
        filepath = os.path.join(output_dir, filename)
        
        if os.path.exists(filepath):
            print(f"[{i+1}/{len(ert_products)}] SKIPPING (already exists): {filename}")
            continue
        ...
        print(f"[{i+1}/{len(ert_products)}] SEARCHING: {search_query}")
