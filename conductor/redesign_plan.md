...
- [ ] **Categorization**: Map products into new industry-based types:
...
- [ ] **URL Mapping**: Ensure every product in `products.json` has a direct `url` field pointing to its page on `https://airgas.my/`. For example for the product name "ANTI IMPACT CUT RESISTANT GLOVE", the button will redirect to "https://airgas.my/product/anti-impact-cut-resistant-glove/" url
...
- [ ] **Grouping Logic**: Modify `renderProducts()` to group products by a `subCategory` field (e.g., grouping all EEBDs under an "EEBD" header).
- [ ] **Dynamic Links**: Ensure the "View more" button correctly uses the `url` from the JSON data.
- [ ] **Animation**: Keep the staggered reveal but adjust for the new grid layout.

## 4. Verification & Testing
- [ ] Verify all "View more" buttons redirect to the correct page on `https://airgas.my/`.
- [ ] Test filtering responsiveness across desktop, tablet, and mobile.
- [ ] Visual check: Compare the final result with `@Web design.jpg` for layout, spacing, and color accuracy.
